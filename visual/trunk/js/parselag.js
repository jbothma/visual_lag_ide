...
var LAGParser = Editor.Parser = (function() {
  ...
  function indentLAG(lexical) {
    return function(firstChars) {
	  ...
	  else if (lexical.type == "init" || lexical.type == "impl")
		return lexical.indented;
  ...
  function parseLAG(input, basecolumn) {
	...
	function condition(type) {
	  if (type == "model") cont(pushlex("stat"), expect("."), dotsep(model)/*attribute*/, optionalcompare/*compare, value*/, poplex);
	  ...
	  else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>GM, UM, PM, DM, Concept, enough, true, false</b>"); cont(); }
	}
	...
	function then(type) {
	  ...
	  else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>then</b>"); cont(); }
	}
	...
	function dostat(type) {
	  ...
	  else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>do</b>"); cont(); }
	}
	function range(type) {
	  if (type == "number") cont();
	  else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>number</b>"); cont(); }
	}
	function sourcelabel(type) {
	  if (type == "variable") cont();
	  else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>variable</b>"); cont(); }
	}
	function model(type) {
	  if (type == "model") cont();
	  else if (type == "variable") cont();
	  else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>DM, GM, UM, PM, Concept, variable</b>"); cont(); }
	}
	/*
	...
	*/
	...
	function compare(type) {
		...
		else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>&lt;, &gt;, ==, !=, in</b>"); cont(); }
	}
	function op(type) {
		if (type == "operator") cont();
		else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>=, .=, +=, -=</b>"); cont(); }
	}
	...
	function value(type) {
		...
		else { mark("lag-error"); error("Found <b style=\"color:red;\">" + type + "</b>: Expected <b>variable, true, false, number, DM, UM, GM, PM, Concept</b>"); cont(); }
	}
  ...
  return {make: parseLAG, electricChars: "()", vars: vars};
})();
