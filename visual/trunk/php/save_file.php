...
//if ($deleteFile != "" && file_exists("users/" . $folder . "/" . $deleteFile)) {
//	unlink("users/" . $folder . "/" . $deleteFile);
//}
...
if ($fileName == "") {
	...
	$fileName = "temp" . time() . ".xml";
}

// If $isPublic is NOT false, create a public file, otherwise create a private file
// Create a relative file path
if ($isPublic != "false") {
	$file = "../users/public/" . $fileName;
} else {
	$file = "../users/" . $folder . "/" . $fileName;
}
...
if ($isPublic != "false") {
	if (file_exists($file)) {
		...
		if ($user != $_SESSION['username']) {
			echo "<span style=\"color: red;\">Could not save file, another user has already created a file with the same name.</span>";
			exit;
		}
	}
}
...
if ($fileHandle) {
	...
	if (fwrite($fileHandle, $xml_string) !== false) {
		fclose($fileHandle);
		echo "<span style=\"color: green;\">File saved OK. Filename: " . $fileName . "</span>";
	} else {
		fclose($fileHandle);
		echo "<span style=\"color: red;\">Could not save file...</span>";
	}
} else {
	echo "<span style=\"color: red;\">Could not save file...</span>";
}
