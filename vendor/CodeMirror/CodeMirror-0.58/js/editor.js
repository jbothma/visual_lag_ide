...
var Editor = (function () {
    ...
    function splitSpaces(string) {
        ...
        else return string.replace(/[\t \u00a0]{2,}/g, function (s) {
    ...
    function traverseDOM(start) {
        ...
        function scanNode(node, c) {
            if (node.nextSibling) c = push(scanNode, node.nextSibling, c);
    ...
    function nodeSize(node) {
        ...
        else return node.currentText.length;
    }
    ...
    function SearchCursor(editor, string, fromCursor) {
        ...
        function () {
            ...
            if (match == -1 || match != firstLine.length - target[0].length) return false;
            var startOffset = self.offset + match;

            var line = self.history.nodeAfter(self.line);
            for (var i = 1; i < target.length - 1; i++) {
                if (cleanText(self.history.textAfter(line)) != target[i]) return false;
                line = self.history.nodeAfter(line);
            }

            if (cleanText(self.history.textAfter(line)).indexOf(target[target.length - 1]) != 0) return false;
    ...
    function Editor(options) {
        ...
        this.win = window;
        this.history = new History(this.container, this.options.undoDepth, this.options.undoDelay, this, options.onChange);

        if (!Editor.Parser) throw "No parser loaded.";
        if (options.parserConfig && Editor.Parser.configure) Editor.Parser.configure(options.parserConfig);

        if (!options.textWrapping) this.container.style.whiteSpace = "pre";
        ...
        if (!options.readOnly) {
            ...
            function setEditable() {
                ...
                if (document.body.contentEditable != undefined && /MSIE/.test(navigator.userAgent)) document.body.contentEditable = "true";
            ...
            if (this.options.autoMatchParens) addEventHandler(document.body, "click", method(this, "scheduleParenBlink"));
        }
    }
    ...
    Editor.prototype = {
        ...
        currentLine: function () {
            var pos = select.cursorPos(this.container, true),
                line = 1;
            if (!pos) return 1;
        ...
        selectedText: function () {
            ...
            if (start.node == end.node) return h.textAfter(start.node).slice(start.offset, end.offset);
        ...
        reindent: function () {
            if (this.container.firstChild) this.indentRegion(null, this.container.lastChild);
        },
        ...
        keyDown: function (event) {
            ...
            if (this.options.autoMatchParens) this.scheduleParenBlink();
        ...
        keyUp: function (event) {
            if (internetExplorer) this.container.createTextRange().execCommand("unlink");

            if (!isSafeKey(event.keyCode)) this.markCursorDirty();
        },
        ...
        indentLineAfter: function (start) {
            ...
            if (whiteSpace && !hasClass(whiteSpace, "whitespace")) whiteSpace = null;
        ...
        handleTab: function () {
            if (this.options.dumbTabs) {
                ...
                if (start == end) this.indentAtCursor();
                else this.indentRegion(start, end);
            }
        },
            ...
            function tryFindMatch() {
                ...
                for (var runner = cursor; runner; runner = dir ? runner.nextSibling : runner.previousSibling) {
                    if (runner.className == className && runner.nodeName == "SPAN" && (ch = paren(runner))) {
                        if (forward(ch) == dir) stack.push(ch);
                        else if (!stack.length) ok = false;
                        else if (stack.pop() != matching[ch]) ok = false;
        ...
        indentAtCursor: function () {
            ...
            if (cursor == lineStart && whiteSpace) cursor = whiteSpace;
            // This means the indentation has probably messed up the cursor.
            if (cursor == whiteSpace) select.focusAfterNode(cursor, this.container);
        },
        ...
        indentRegion: function (current, end) {
            var sel = select.markSelection(this.win);
            if (!current) this.indentLineAfter(current);
            else current = startOfLine(current.previousSibling);
            end = startOfLine(end);
        ...
        reparseBuffer: function () {
            ...
            if (this.container.firstChild) this.addDirtyNode(this.container.firstChild);
        },
        ...
        addDirtyNode: function (node) {
            ...
            if (node.nodeType != 3) node.dirty = true;
        ...
        getDirtyNode: function () {
            while (this.dirty.length > 0) {
                ...
                try {
                    ...
                    if ((found.dirty || found.nodeType == 3) && found.parentNode) return found;
                } catch(e) {}
            }
            return null;
        },
        ...
        highlightDirty: function (force) {
            ...
            while (lines > 0 && (start = this.getDirtyNode())) {
                ...
                if (result) {
                    lines = result.left;
                    if (result.node && result.dirty) this.addDirtyNode(result.node);
                }
            }
            select.selectMarked(sel);
        ...
        documentScanner: function (linesPer) {
            ...
            return function () {
                ...
                // well, we start over.
                if (pos && pos.parentNode != self.container) pos = null;
        ...
        highlight: function (from, lines, cleanLines) {
            ...
            if (from && !from.nextSibling) return;
            ...
            var parts = {
                ...
                // Fetch current node.
                get: function () {
                    if (!this.current) this.current = traversal.nodes.shift();
                    return this.current;
                },
            ...
            forEach(parsed, function (token) {
                ...
                if (token.value == "\n") {
                    ...
                    if (part.nodeName != "BR") throw "Parser out of sync. Expected BR.";
                    ...
                    if ((lines !== undefined && --lines <= 0) || (!lineDirty && lineNodes > 1 && !cleanLines)) throw StopIteration;
                    lineDirty = false;
                    lineNodes = 0;
                    parts.next();
                } else {
                    if (part.nodeName != "SPAN") throw "Parser out of sync. Expected SPAN.";
                    if (part.dirty) lineDirty = true;
                    lineNodes++;
