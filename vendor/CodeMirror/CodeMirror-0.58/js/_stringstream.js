    ...
    var base = {
        ...
        matches: function (string, caseSensitive) {
            for (var i = 0; i < string.length; i++) {
                var ch = this.peek();
                if (!ch || string.charAt(i) != (caseSensitive ? ch : ch.toLowerCase())) return false;
                this.next();
            }
            return true;
        }
        ...
        return update({
            ...
            next: function () {
                try {
                    ...
                    if (e == StopIteration && accum.length > 0) throw "End of stringstream reached without emptying buffer ('" + accum + "').";
                    else throw e;
                }
            },
            get: function () {
